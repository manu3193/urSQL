...
/**
 * Clase encargada de de borrar elimina las filas de la tabla que cumple la 
 * condición del where.
 * @author JoséAlberto
 */
public class Delete {
     ...
     * @param value 
     */
    public Delete(String columns, Table tables,String newValue, String colCond, String Ope, int value){// caso con where
                        ...
                        if (tables.getRows().get(0).getColumns()
                            ...
                            for (int countRow = 0; countRow < tables.getRows().size(); countRow++) { //navegar por toda la columna de una tabla en específico
                                if (verifyCond(tables.getRows().get(countRow).getColumns().get(columnWhere).getContent())){//método para boolean condicion
                                    tables.getRows().get(countRow);//falta para eliminar fila
